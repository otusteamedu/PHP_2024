...
* add - добавить событие (событие передается в параметре data в виде json-строки)
* clear - очистить все события в хранилище
* get - получить событие по заданным условиям (условия передаются в параметре data в виде json-строки)

### Примеры запросов
* добавление 

http://mysite.local?command=add&data={ "priority": 1000, "conditions": { "param1":1 }, "event": "Event1" }
http://mysite.local?command=add&data={ "priority": 3000, "conditions": { "param1":2, "param2":2 }, "event": "Event2" }
http://mysite.local?command=add&data={ "priority": 3000, "conditions": { "param1":1, "param2":2 }, "event": "Event3" }

* удаление

http://mysite.local?command=clear

* получение по заданным условиям

http://mysite.local?command=get&data={"params": { "param1": 1, "param2": 2 }}