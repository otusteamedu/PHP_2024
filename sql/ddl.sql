DROP TABLE IF EXISTS tickets, random_tickets_with_base_prices, seats, sessions, films, halls;

CREATE TABLE IF NOT EXISTS halls
(
    id    SERIAL PRIMARY KEY,
    title VARCHAR(50) NOT NULL
);

CREATE TABLE IF NOT EXISTS seats
(
    id      SERIAL PRIMARY KEY,
    num     INT NOT NULL,
    row     INT NOT NULL,
    hall_id INT NOT NULL REFERENCES halls (id)
);

CREATE TABLE IF NOT EXISTS films
(
    id          SERIAL PRIMARY KEY,
    title       VARCHAR(50) NOT NULL,
    start_date  DATE,
    end_date    DATE,
    rental_cost FLOAT       NOT NULL
);

CREATE TABLE IF NOT EXISTS sessions
(
    id         SERIAL PRIMARY KEY,
    start_time TIME NOT NULL
);

CREATE TABLE IF NOT EXISTS random_tickets_with_base_prices
(
    id         SERIAL PRIMARY KEY,
    hall_id    INT   NOT NULL REFERENCES halls (id),
    film_id    INT   NOT NULL REFERENCES films (id),
    session_id INT   NOT NULL REFERENCES sessions (id),
    base_price FLOAT NOT NULL
);

CREATE TABLE IF NOT EXISTS tickets
(
    random_tickets_with_base_prices_id INT  NOT NULL REFERENCES random_tickets_with_base_prices (id),
    seat_id                            INT  NOT NULL REFERENCES seats (id),
    session_date                       DATE NOT NULL,
    real_price                         FLOAT
);

-- Индексы улучшения
-- CREATE INDEX tickets_session_date_idx ON tickets (session_date);
-- CREATE INDEX sessions_start_time_idx ON sessions (start_time);
-- CREATE INDEX films_start_date_idx ON films (start_date);
-- CREATE INDEX films_end_date_idx ON films (end_date);
-- CREATE INDEX films_title_idx ON films (title);
-- CREATE INDEX halls_title_idx ON halls (title);