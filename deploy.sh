...
DEPLOY_PATH="/var/www/health-diary/releases"
CURRENT_PATH="/var/www/health-diary/current"
PREVIOUS_PATH="/var/www/health-diary/previous"
NGINX_CONFIG_PATH="/var/www/health-diary/current/infra/nginx/health-diary" # Путь к конфигу Nginx в проекте
NGINX_ENABLED_PATH="/etc/nginx/sites-enabled/health-diary"                           # Путь к sites-enabled конфигу Nginx
NGINX_AVAILABLE_PATH="/etc/nginx/sites-available/health-diary"                       # Путь к sites-available конфигу Nginx
...
git clone git@gitlab.com:Va1demar/health-diary.git $NEW_RELEASE_PATH

# Производим composer install
cd $NEW_RELEASE_PATH && composer install --no-dev --optimize-autoloader
...
# Удаляем все старые релизы, кроме последних 3
RELEASES_COUNT=5
cd $DEPLOY_PATH
