...
select object_name, pg_size_pretty(object_size) as size, type
from (SELECT
    relname as object_name, pg_relation_size(relid) as object_size, 'table' as type
    FROM pg_catalog.pg_statio_user_tables
    union
    SELECT ii.indexname as object_name, PG_INDEXES_SIZE(relid) AS object_size, 'index' as type
