...
namespace AlexanderGladkov\Bookshop\Config;
...
abstract class BaseConfig
{
    ...
    protected function readSection(string $sectionName, array $fields): void
    {
        ...
        foreach ($fields as $configFieldName) {
            if (empty($section[$configFieldName])) {
                throw new ConfigValidationException($this->getFieldIsEmptyErrorMessage($configFieldName, $sectionName));
            }

            $this->setFiledValue(
                $this->convertSnakeCaseStringToCamelCaseString($configFieldName),
                $section[$configFieldName]
            );
        }
    }
