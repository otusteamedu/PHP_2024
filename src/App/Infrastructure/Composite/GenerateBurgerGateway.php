...
class GenerateBurgerGateway implements FoodCompositeInterface
{
    public function composite(FoodCompositeRequest $request): FoodCompositeResponse
    {
        ...
        $food->add(new Cutlet('Говяжья котлета'));
        $food->add(new Salad('Салат'));

        $sauce = new Sauce('Чесночный соус');
        $sauce->add(new Garlic('Чеснок'));
